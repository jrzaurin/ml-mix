...
if not sys.platform.startswith("win32"):  # noqa: C901
...
def download(  # noqa: C901
    ...
    assert (
        retries >= 0
    ), "Number of retries should be at least 0, currently it's {}".format(retries)
    ...
    if (
        ...
        or (sha1_hash and not sha1sum(fname) == sha1_hash)
    ):
            ...
            try:
                    ...
                    if tqdm is not None:
                        ...
                        with tqdm.tqdm(
                            ...
                            s3.meta.client.download_file(
                                s3_bucket_name, s3_key, tmp_path, Callback=hook(t)
                            )
                    else:
                ...
                if not os.path.exists(fname) or (
                    ...
                    try:
                        ...
                        warnings.warn(
                            "File {} exists in file system so the downloaded file is deleted".format(
                                fname
                            )
                        )
