...
def preprocess(
    df,
    with_tax_values=True,
    log_scale_lot=True,
    log_scale_listed_price=True,
    has_label=True,
):
    new_df = df.copy()
    new_df["Elementary School"] = new_df["Elementary School"].apply(
        lambda ele: str(ele)[: -len(" Elementary School")]
        if str(ele).endswith("Elementary School")
        else ele
    )
    if log_scale_lot:
...
all_train_df = preprocess(
    all_train_df,
)

os.makedirs(os.path.join(args.dir_path, "processed"), exist_ok=True)
train_df, test_df = train_test_split(
    all_train_df, test_size=0.2, random_state=np.random.RandomState(seed)
)
...
competition_df.to_csv(
    os.path.join(args.dir_path, "processed", "competition.csv"), index=False
)
print(
    f"#Train={len(train_df)}, #Dev={len(test_df)}, #Competition={len(competition_df)}"
)
