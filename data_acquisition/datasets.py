...
class MachineHackSentimentPrediction(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._path = os.path.join(
            get_data_home_dir(), "machine_hack_sentiment_analysis", f"{split}.csv"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class JigsawUnintendedBias(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        self._split = split
        self._path = os.path.join(get_data_home_dir(), self._LOCAL_NAME, f"{split}.pq")
        download(
            self._INFO[split]["url"],
            path=self._path,
            sha1_hash=self._INFO[split]["sha1sum"],
    ...
    def feature_columns(self):
        return (
            self._text_columns
            + self._identity_attribute_columns
            + self._user_voting_columns
        )

    @property
    def feature_types(self):
        return [_TEXT] * len(self._text_columns) + [_NUMERICAL] * (
            len(self._identity_attribute_columns) + len(self._user_voting_columns)
        )
...
class GoogleQuestQALabel(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        self._split = split
        self._path = os.path.join(get_data_home_dir(), "google_quest_qa", f"{split}.pq")
        download(
            self._INFO[split]["url"],
            path=self._path,
            sha1_hash=self._INFO[split]["sha1sum"],
...
class WomenClothingReview(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "women_clothing_review", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class MelBourneAirBnb(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "airbnb_melbourne", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
    ...
    def feature_columns(self):
        ...
        feature_columns = [
            ...
            if col not in self.label_columns and col not in self.ignore_columns
        ]
...
class AEPricePrediction(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "ae_price_prediction", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class IMDBGenrePrediction(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "imdb_genre_prediction", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class FakeJobPostings(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "fake_job_postings", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class KickStarterFunding(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "kick_starter_funding", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class JCPennyCategory(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "jc_penney_products", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class WineReviews(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        super().__init__()
        self._split = split
        self._path = os.path.join(get_data_home_dir(), "wine_reviews", f"{split}.pq")
        download(
            self._INFO[split]["url"],
            path=self._path,
            sha1_hash=self._INFO[split]["sha1sum"],
...
class NewsPopularity(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        super().__init__()
        self._split = split
        self._path = os.path.join(get_data_home_dir(), "news_popularity", f"{split}.pq")
        download(
            self._INFO[split]["url"],
            path=self._path,
            sha1_hash=self._INFO[split]["sha1sum"],
...
class NewsPopularity2(NewsPopularity):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "news_popularity2", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class FakeJobPostings2(FakeJobPostings):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "fake_job_postings2", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class BookPricePrediction(BaseMultiModalDataset):
    ...
    _INFO = {
        "train": {
            "url": get_repo_url()
            + "machine_hack_competitions/predict_the_price_of_books/train.csv",
            "sha1sum": "781ec11238d5b15f3803ee7575b12e0b1b982e0d",
        },
        "test": {
            "url": get_repo_url()
            + "machine_hack_competitions/predict_the_price_of_books/test.csv",
            "sha1sum": "7b1aa0526a5c8e9a89e5da0aeb77772317bd93c9",
        },
        "competition": {
            "url": get_repo_url()
            + "machine_hack_competitions/predict_the_price_of_books/competition.csv",
...
class DataScientistSalaryPrediction(BaseMultiModalDataset):
    ...
    _INFO = {
        "train": {
            "url": get_repo_url()
            + "machine_hack_competitions/predict_the_data_scientists_salary_in_india_hackathon/train.csv",
            "sha1sum": "9ea3bb2bcd0396ae6c70d62bad32ef211276b0f6",
        },
        "test": {
            "url": get_repo_url()
            + "machine_hack_competitions/predict_the_data_scientists_salary_in_india_hackathon/test.csv",
            "sha1sum": "98194e41d1d4ebb251b38c2d474f486b08769a9f",
        },
        "competition": {
            "url": get_repo_url()
            + "machine_hack_competitions/predict_the_data_scientists_salary_in_india_hackathon/competition.csv",
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "data_scientist_salary", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
...
class KaggleCaliforniaHousePricePrediction(BaseMultiModalDataset):
    ...
    def __init__(self, split="train"):
        ...
        self._split = split
        self._path = os.path.join(
            get_data_home_dir(), "california_house_price", f"{split}.pq"
        )
        download(
            self._INFO[split]["url"],
            path=self._path,
    ...
    def feature_columns(self):
        return [
            ...
            if col not in self.label_columns and col != "Id"
        ]
