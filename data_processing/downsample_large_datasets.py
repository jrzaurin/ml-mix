...
sampled_train_data = train_dataset.data.sample(
    n=100000, random_state=np.random.RandomState(seed)
)
sampled_test_data = test_dataset.data.sample(
    n=int(len(test_dataset.data) / len(train_dataset.data) * 100000), random_state=seed
)
os.makedirs("jigsaw_unintended_bias_sampled")
sampled_train_data.to_parquet(
    os.path.join("jigsaw_unintended_bias_sampled", "train.pq")
)
...
sampled_train_data.to_parquet(
    os.path.join("mercari_price_suggestion_sampled", "train.pq")
)
sampled_test_data.to_parquet(
    os.path.join("mercari_price_suggestion_sampled", "test.pq")
)
