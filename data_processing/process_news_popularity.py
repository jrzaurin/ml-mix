...
while upper_index < num_scrape:
    ...
    pickle.dump(
        article_titles,
        open(os.path.join(directory, output_subdir, "article_titles.p"), "wb"),
    )
    j += 1
...
train_data, test_data = train_test_split(
    all_train_data, test_size=0.2, random_state=np.random.RandomState(seed)
)
...
#     label=label, path=directory + output_subdir, problem_type=problem_type
# )
# predictor.fit(
#     train_data,
