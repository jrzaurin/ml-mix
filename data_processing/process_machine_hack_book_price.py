...
# Convert Reviews
all_train_df.loc[:, "Reviews"] = pd.to_numeric(
    all_train_df["Reviews"].apply(lambda ele: ele[: -len(" out of 5 stars")])
)
competition_df.loc[:, "Reviews"] = pd.to_numeric(
    competition_df["Reviews"].apply(lambda ele: ele[: -len(" out of 5 stars")])
)
# Convert Ratings
all_train_df.loc[:, "Ratings"] = pd.to_numeric(
    all_train_df["Ratings"].apply(
        lambda ele: ele.replace(",", "")[: -len(" customer reviews")]
    )
)
competition_df.loc[:, "Ratings"] = pd.to_numeric(
    competition_df["Ratings"].apply(
        lambda ele: ele.replace(",", "")[: -len(" customer reviews")]
    )
)
